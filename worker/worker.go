...
func hydrateBatch(
	...
	rt *runtime.Runtime,
	batch []tasks.Task,
) (docsByType map[string]map[string]string, assetsByType map[string]map[string][]vl.AssetURL, err error) {
	...
	for _, t := range batch {
		if strings.TrimSpace(t.EntityType) == "" || strings.TrimSpace(t.EntityID) == "" {
			continue
		}
		if _, ok := idsByType[t.EntityType]; !ok {
			idsByType[t.EntityType] = map[string]struct{}{}
		}
		idsByType[t.EntityType][t.EntityID] = struct{}{}
...
func handleTaskResult(
	...
	if err == nil || errors.Is(err, runtime.ErrEntityNotFound) {
		_ = repo.Complete(ctx, task.EntityType, task.EntityID, task.Model, task.NextRunAt)
		return
	}
	...
	_ = repo.Fail(ctx, task.EntityType, task.EntityID, task.Model, task.NextRunAt, backoff)
}

func processBatch(ctx context.Context, rt *runtime.Runtime, repo *tasks.Repo, cfg Options, batch []tasks.Task, docsByType map[string]map[string]string, assetsByType map[string]map[string][]vl.AssetURL, sem chan struct{}, tokens <-chan struct{}, rng *rand.Rand) {
	...
	for _, task := range batch {
		doc := ""
		if m, ok := docsByType[task.EntityType]; ok {
			doc = m[task.EntityID]
		}
	...
	for _, it := range vlItems {
		...
		go func() {
			...
			err := rt.GenerateAndStoreVLEmbeddingWithInputs(ctx, it.task.EntityType, it.task.EntityID, it.task.Model, it.doc, it.assets)
			handleTaskResult(ctx, repo, cfg, rng, it.task, err)
		}()
	}
...
//
// This is useful for integrating embeddingkit into an external job runner (e.g.
